...
using _08_medical_rag_system;
using _08_medical_rag_system.Infrastructure;
...
Console.WriteLine("=== RAG-Based Multi-Agent Medical System (08-medical-rag-system) ===");
Console.ResetColor();
...
try
{
    ...
    var knowledgeBase = new MedicalKnowledgeBase();

    patientRegistry.Initialize();
    Console.WriteLine("Database initialized (hospital.db).");

    // 4. Seed default medical knowledge base if file doesn't exist
    var acronymsPath = knowledgeBase.GetAcronymsFilePath();
    if (!File.Exists(acronymsPath))
    {
        ...
        File.WriteAllText(acronymsPath, """
            Hypertension -> HTA
            ...
            Heart Failure -> ICC
            Cardiac Insufficiency -> ICC
            Dyslipidemia -> DL
            Hyperlipidemia -> DL
            High Cholesterol -> DL
            Atrial Fibrillation -> FA
            Auricular Fibrillation -> FA
    ...
    {
        Console.WriteLine($"✓ Medical knowledge base loaded: {acronymsPath}");
    }
    ...
    AIAgent medicalDataAnalyst = chatClient.CreateAIAgent(
        ...
        tools:
        [
            AIFunctionFactory.Create(knowledgeBase.SearchMedicalKnowledge)
        ]
    );
